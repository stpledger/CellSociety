...
public class StandardGridWrap extends StandardGrid {
	...
	public void assignNeighborsWrap(int row, int col){
		for(int i = 0; i < col; i++){
			getCellMap().get(new Point(i, 0)).addNeighbor(getCellMap().get(new Point(i, row - 1)));
		}
		for(int i = 0; i < col; i++){
			getCellMap().get(new Point(i, row - 1)).addNeighbor(getCellMap().get(new Point(i, 0)));
		}
		for(int i = 0; i < row; i++){
			getCellMap().get(new Point(0, i)).addNeighbor(getCellMap().get(new Point(col - 1, i)));
		}
		for(int i = 0; i < row; i++){
			getCellMap().get(new Point(col - 1, i)).addNeighbor(getCellMap().get(new Point(0, i)));
		}
	}
}
